...
const navigateTicketForm = () => {
 ...
 const filterData =(e) => {
        if(e.target.value !=""){
            ...
            const filterTable = ticket.filter(o=>Object.keys(o).some(k=>String(o[k]).toLowerCase().includes(e.target.value.toLowerCase())));
...
return (
  ...
  <input type="text" class="form-control" placeholder="Search" aria-label="Username" aria-describedby="basic-addon1" value={value}
   onChange={filterData}/>
        ...
        <section id='Mytickets' className={`flex md:flex-row flex-col ${styles.paddingY}`}>
                        ...
                        <thead className= {`${styles.ALthread}`}>
                            <tr>
                                <th scope="col" className= {`${styles.ALth}`}>
                                    <button className= {`${styles.ALbtn} font-semibold`} onClick={ navigateTicketForm }>
                                        Add New Ticket
                                    </button>
      ...
      <button
        ...
        >
                        ...
                        <tbody>
                            {value.length > 0 ? filterTable.map((doc, index) =>{
                                return(
                                    <tr key={doc.id}>
                                        <td className= {`${styles.ALtd}`}> <a href="/Adminticket">{index + 1}</a></td>
                                        <td className= {`${styles.ALtd}`}>{doc.subject}</td>
                                        <td className= {`${styles.ALtd}`}>{doc.issueDate}</td>
