...
const StockList = ({getStockId}) => {
  ...
  return (
    <Helmet title='Stock List'>
      ...
      <input type="text" id="search" placeholder='Search Stock' onChange={(e)=> setSearch(e.target.value)} value={search} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
        
      </form>
      <PDFExport ref={pdfExportComponent} paperSize='A1'>
      <div className={`flex-1 ${styles.flexStart} flex-col xl:px-0 sm:px-16 px-6 SL-bg-table rounded-md SL-log`}>
        <div>
        <button className={`${styles.ADbtn }`} onClick={navigateStockForm} >Add New Stock</button>
        <button className={`${styles.ADbtn}`} onClick={handleExportWithComponent}>Take a Snapshot</button>
        </div>
          ...
          <tbody>
            {Stock.map((doc,index)=>{
              ...
              <tr key={doc.id} >
                ...
                <td>
                  <button className= {`${styles.SLbtn}`}><Link to={`stockupdateform/${doc.id}`}>Edit</Link></button>
